...
Each test is being run two times, so you can see how caching works (look at example at the bottom of this doc).
...
Testing one of my applications.
    test 'it loads albums of first user' do
      User.all.first.albums.collect
    end
...
Results
    ...
    Testing it does some time expensive operation
...
Copyright (c) 2011 Dawid Sklodowski, released under the MIT license